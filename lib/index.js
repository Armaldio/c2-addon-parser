...
function markdown_export (json) {
    ...
    md.add('#### There ' + plural + ' ' + actions.length + ' action' + ((actions.length > 1) ? 's' : '') + ' available');
    ...
    md.add('#### There ' + plural + ' ' + conditions.length + ' condition' + ((actions.length > 1) ? 's' : '') + ' available');
    ...
    md.add('#### There ' + plural + ' ' + expressions.length + ' expression' + ((actions.length > 1) ? 's' : '') + ' available');
    ...
    _.forEach(expressions, function (value, index) {
        ...
        _.forEach(params, function (v, i) {
            var defaultValue = (v.defaultValue !== '') ? ' (default value : ' + v.defaultValue + ')' : '';
            if (v.text === '') {
                md.add('* Param ' + (i + 1) + " : " + v.description + defaultValue);
            } else {
                md.add('* Param ' + (i + 1) + ' : **' + v.text + '** : ' + v.description + defaultValue);
            }
            if (v.options.length >= 1) {
                _.forEach(v.options, function (value, index) {
                    md.add('  * ' + value);
